...
class HangmanGame {
public:
	static void updatePlayer(osucat::SenderInfo info, osucat::Target tar) {
		try {
			Database db;
			db.Connect();
	...
	static bool playerRankingExists(osucat::SenderInfo info, osucat::Target tar) {
		try {
			Database db;
			db.Connect();
	...
	static void showPlayerData(osucat::SenderInfo info, osucat::Target tar, string* out) {
	...
	//开始一局游戏的请求
	static void startHangman(osucat::SenderInfo senderinfo, osucat::Target tar, string cmd, string *out) {
		*out = "";
		char mods = 0x00;
	...
	//处理游戏的输入
	static void inputHangman(osucat::SenderInfo senderinfo, osucat::Target tar, string input, string *out) {
	...
	//放弃一局游戏
	static void giveupHangman(osucat::SenderInfo senderinfo, osucat::Target tar, string* out) {
		int position = findPlayer(tar.user_id);
		if (position == -1) {
