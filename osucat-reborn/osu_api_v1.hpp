...
#define OSU_KEY "eb690a90a52cf87e7998ae9e813d71c948b7d862"
...
namespace osucat::osu_api_v1 {
    ...
    public:
        ...
        static int GetUser(int64_t uid, mode mode, user_info* info) {
            char url[512];
            sprintf_s(url, 512, OSU_API_V1 "get_user?type=id&k=" OSU_KEY "&u=%I64d&m=%d", uid, (int)mode);
        ...
        static int GetUser(string username, mode mode, user_info* info) {
            char url[512];
            sprintf_s(url, 512, OSU_API_V1 "get_user?type=string&k=" OSU_KEY "&u=%s&m=%d", username.c_str(), (int)mode);
            string response = NetConnection::HttpsGet(url);
            if (response.length() > 50) {
                json data = json::parse(response)[0];
                tm time_temp;
        ...
        static int GetBeatmap(int64_t bid, beatmap_info* info) {
            char url[512];
            sprintf_s(url, 512, OSU_API_V1 "get_beatmaps?k=" OSU_KEY "&b=%I64d", bid);
            string response;
        ...
        static int GetBeatmapset(int64_t sid, vector<beatmap_info>& info) {
            info.clear();
            char url[512];
            sprintf_s(url, 512, OSU_API_V1 "get_beatmaps?k=" OSU_KEY "&s=%I64d", sid);
            string response;
            response = NetConnection::HttpsGet(url);
            if (response.length() > 50) {
        ...
        static int GetUserRecent(int64_t uid, mode mode, score_info* info) {
            char url[512];
            sprintf_s(url, 512, OSU_API_V1 "get_user_recent?type=id&k=" OSU_KEY "&u=%I64d&m=%d", uid, (int)mode);
            string response = NetConnection::HttpsGet(url);
            if (response.length() > 50) {
                json data = json::parse(response)[0];
        ...
        static int GetUserRecent(const string& username, mode mode, score_info* info) {
            ...
            sprintf_s(url,
                512,
                OSU_API_V1 "get_user_recent?type=string&k=" OSU_KEY "&u=%s&m=%d",
                username.c_str(),
                (int)mode);
            string response;
            response = NetConnection::HttpsGet(url);
        ...
        static int GetUserBest(int64_t uid, int count, mode mode, vector<score_info>& info) {
            ...
            sprintf_s(url,
                512,
                OSU_API_V1 "get_user_best?type=id&k=" OSU_KEY "&u=%I64d&m=%d&limit=%d",
                uid,
                (int)mode,
                count);
            string response;
        ...
        static int GetUserBest(const string& username, int count, mode mode, vector<score_info>& info) {
            ...
            sprintf_s(url,
                512,
                OSU_API_V1 "get_user_best?type=string&k=" OSU_KEY "&u=%s&m=%d&limit=%d",
                username.c_str(),
                (int)mode,
                count);
            string response;
        ...
        static int GetUserScore(int64_t uid, mode mode, int64_t beatmapid, score_info* info) {
            ...
            sprintf_s(url,
                512,
                OSU_API_V1 "get_scores?type=id&k=" OSU_KEY "&u=%lld&m=%d&limit=%d&b=%lld",
                uid,
                (int)mode,
                1,
                beatmapid);
